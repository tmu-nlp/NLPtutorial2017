...
def train_rnn():
    ...
    with open("../../data/wiki-en-train.norm_pos") as f:
        f = list(f)
...
def gradient_rnn(hidden_layer, output_layer, x_list, y_list, h_list, p_list, vocab_size, output_size, middle_node=8):
    ...
    delta_r_ = np.zeros((middle_node, 1))  # error from next time step

    for t in range(l-1, -1, -1):
        _delta_o = y_list[t] - p_list[t]        
        delta_w_oh += np.outer(h_list[t], _delta_o)  # memo (2,1)*(42,1)
        delta_b_o += _delta_o
        delta_r = np.dot(hidden_layer[1], delta_r_) + np.dot(output_layer[0], _delta_o)  # delta_r.shape=(2,1)
        delta_r_ = delta_r * (1-pow(h_list[t], 2)) 
        delta_w_rx += np.outer(x_list[t].T, delta_r_)  # memo (4700,1)*(2,1)
        delta_b_r += delta_r_
        if t != 0:
            delta_w_rh += np.outer(h_list[t-1], delta_r_)  # memo (2,1)*(2,1)
...
def forward_rnn(h_layer, o_layer, x_vector, h_list, t):
    ...
    p_vector = softmax(np.dot(o_layer[0].T, h_list[-1])+o_layer[1])  # softmax
...
def init_network(middle_node=8):
    ...
    with open("../../data/wiki-en-train.norm_pos") as f:
        ...
        vocab_size = len(x_ids)
        output_size = len(y_ids)
        hidden_layer = list()  # (w_rx, w_rh, b_r)
        output_layer = list()  # (w_oh, b_o)
        hidden_layer.append(np.random.uniform(-0.1, 0.1, (vocab_size, middle_node)))  # w_rx
        hidden_layer.append(np.random.uniform(-0.1, 0.1, (middle_node, middle_node)))  # w_rh
        hidden_layer.append(np.zeros((middle_node, 1)))  # b_r
        output_layer.append(np.random.uniform(-0.1, 0.1, (middle_node, output_size)))  # w_oh
        output_layer.append(np.zeros((output_size, 1)))  # b_o
